...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(e => new
            {
                e.EName,
                Total = e.Sal + (e.Comm ?? 0) // Jeśli Comm jest null, traktujemy je jako 0
            })
            .ToList();
...
//nie dziala
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, 
                e => e.DeptNo,    // Łączymy Emp.DeptNo z Dept.DeptNo
                d => d.DeptNo,    
                (e, d) => new { e.EName, d.DName, e.Sal }) // Dodajemy Sal do wyniku
            .Join(grades, 
                ed => ed.Sal,    // Łączymy Emp.Sal z Salgrade.Losal
                s => s.Losal,    // Używamy Losal w celu znalezienia odpowiedniego przedziału
                (ed, s) => new { ed.EName, ed.DName, ed.Sal, Grade = s.Grade })
            .Where(r => r.Sal >= r.Grade && r.Sal <= grades.FirstOrDefault(g => g.Grade == r.Grade)?.Hisal) 
            .ToList();

        // Debugging step: Show the result to check the data
        foreach (var item in result)
        {
            Console.WriteLine($"Employee: {item.EName}, Department: {item.DName}, Salary: {item.Sal}, Grade: {item.Grade}");
        }

        // Assert to check if "ALLEN" with "SALES" and Grade 3 exists
