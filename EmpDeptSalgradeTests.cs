...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => chicagoDeptNos.Contains(emp.DeptNo)).ToList();
    ...
    {
        var emps = Database.GetEmps();
        var avgSalaries = emps.GroupBy(e => e.DeptNo)
            .ToDictionary(g => g.Key, g => g.Average(e => e.Sal));

        var result = emps.Where(e => e.Sal > avgSalaries[e.DeptNo])
            .Select(e => e.EName)
            .ToList();
